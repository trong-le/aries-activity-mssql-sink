...
export default class AlterSchemaStrategy extends SchemaStrategy {
    ...
    async prepare(config, incomingSchema) {
        ...
        await this.db.schema.createTableIfNotExists(config.table, this.getSchemaBuilder(incomingSchema));
    ...
    async createMissingColumns(config, currentSchema, incomingSchema) {
        ...
        // for (const field of newFields) {
            await this.db.schema.table(config.table, this.getSchemaBuilder(newFields));
        // }
    ...
    async alterChangedColumns(config, currentSchema, incomingSchema) {
        ...
        await this.db.transaction(async function(trx) {
            ...
            for (const field of changedFields) {
                ...
                await trx.schema.table(config.table, self.getSchemaBuilder([tempField]));
            }
            ...
            await trx.raw(`UPDATE ${config.table} SET ${sets.join(',')}`);
            self.log.info('Copied original values to temp columns.');

            // Drop original columns.
            await trx.schema.table(config.table, table => {
